...
export default function Quiz() {
  ...
  // Fetch subjects, classes, and states from API
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        const [subjectsRes, classesRes, statesRes] = await Promise.all([
          api.get("/allsubjects"),
          api.get("/allclasses"),
          api.get("/states")
        ]);
        ...
        <Form.Item name="bundesland" label="State">
          <Select 
            ...
            options={states.map(s => {
              const stateName = s.state_name || s.name || String(s);
        ...
        <Form.Item name="subject" label="Subject">
          <Select 
            ...
            options={subjects.map(s => {
              ...
              return { value: subjectName, label: subjectName };
            })}
            style={{ minWidth: 160 }}
          />
        </Form.Item>
          ...
          items={[
            {
              ...
              children: (
                <Form
                  layout="vertical"
                  form={editForm}
                  initialValues={{ status: "draft", difficulty: "easy", objectives: [], items: [] }}
                >
                  ...
                  <Form.Item name="subject" label="Subject" rules={[{ required: true }]}>
                    <Select 
                      ...
                      notFoundContent={subjects.length === 0 ? "Loading subjects..." : "No subjects found"}
                    />
                  </Form.Item>
                  <Form.Item name="grade" label="Class" rules={[{ required: true }]}>
                    <Select 
                      showSearch
                      ...
                      notFoundContent={classes.length === 0 ? "Loading classes..." : "No classes found"}
                    />
                  </Form.Item>
