// src/pages/auth/SignUpAddChild.jsx
...
export default function SignUpAddChild() {
  ...
  const handleAddChild = () => {
    try {
      ...
      sessionStorage.setItem(ONBOARDING_RETURN_KEY, "/signup/add-child/another");
    } catch {
      // ignore storage errors (private mode, etc.)
    }
    navigate(CTA_ROUTE, {
      replace: true,
      state: { from: "/signup/add-child", back: location.state?.back || "/signup/success" },
    });
  };
    ...
    >
      ...
      <div className="relative z-10 flex w-full max-w-[520px] flex-col items-center px-6 text-center">
        ...
        <p className="mt-6 text-2xl font-semibold text-[#7DB533] leading-tight">
          Lege jetzt einen Account<br />für Dein Kind an
        </p>

        <p className="mt-6 text-base text-[#6F5A4A]">
          Damit Dein Kind selbstständig und sicher die App nutzen kann, hast Du jetzt die
          Möglichkeit einen geschützten Kinderaccount anzulegen.
        </p>

        <button
          type="button"
          onClick={handleAddChild}
          className="mt-8 flex w-full max-w-[260px] items-center justify-center gap-3 rounded-[28px] bg-white/85 px-4 py-4 text-lg font-semibold text-[#FF8400] shadow-[0_18px_38px_rgba(255,132,0,0.25)] transition hover:bg-white"
        >
          <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#FF8400] text-white shadow-[0_8px_16px_rgba(255,132,0,0.35)]">
            +
          </span>
          Kind hinzufügen
        </button>

        <p className="mt-6 max-w-[380px] text-xs text-[#9A8576]">
          DSGVO konform. Die Daten liegen sicher verschlüsselt auf einem Server in Deutschland und
