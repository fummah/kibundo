...
/* ----------------------------- Dummy Data ----------------------------- */
/** Imagine the “picture” shows tabs for Weekly / Monthly / Yearly
 *  and a toggle for 1 vs 2 children, with featured highlighting. */
const PLANS = [
  // Weekly
  {
    id: "w_1",
  ...
  return (
    <Card className="rounded-3xl border-0 shadow">
      ...
      <p className="mt-3 text-xs text-neutral-500">
        Demo data. Link to admin‑managed plans in production.
      </p>
    </Card>
  );
}
...
export default function Subscription() {
  ...
  const allForComparison = useMemo(() => {
    // compare the visible subset only (like the picture with current tab)
    return filtered;
  }, [filtered]);
  ...
  return (
    <GradientShell>
      ...
      <div className="px-5 md:px-8">
        ...
        {/* Responsive Layout:
            - Mobile: stacked (cards -> promo)
            - Desktop (md+): 3-col plan grid left + sticky summary right + comparison table */}
        <div className="mt-5 grid grid-cols-1 gap-6 lg:max-w-6xl xl:max-w-7xl mx-auto md:grid-cols-3">
          ...
          <div className="md:col-span-2">
            ...
            {/* Desktop grid: 3 columns for a professional, airy look */}
            ...
            <p className="hidden md:block mt-3 text-xs text-white/80">
              Demo data shown. In production, load plans and promos from the admin backend.
            </p>
          </div>

          {/* RIGHT: sticky promo summary */}
