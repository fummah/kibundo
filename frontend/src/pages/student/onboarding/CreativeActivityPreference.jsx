// src/pages/student/onboarding/CreativeActivityPreference.jsx
// Based on fortai_app/src/pages/onboarding/CreativeActivityPreference.jsx
...
const CreativeActivityPreference = () => {
  ...
  const getSafeBuddyImage = () => {
    // Default Kibundo buddy image
    const defaultBuddyImage = "/images/img_kibundo.png";
    
    if (!buddy) return defaultBuddyImage;
    
    // Get the image path from buddy
    const buddyImage = buddy.img || buddy.avatar || "";
    
    // Check if the image path contains character image patterns
    const isCharacterImage = 
      buddyImage.includes("img_rectangle_20") ||
      buddyImage.includes("rectangle_20") ||
    ...
    // If it's character data or character image, return default
    if (isCharacterImage || isCharacterData) {
    ...
    // If we have a valid buddy image, use it
    if (buddyImage && !isCharacterImage) {
      return buddyImage;
    }
  ...
  const textToSpeak = "Malst Du lieber, oder baust Du lieber?";
  ...
  const handleOptionSelect = async (option) => {
    ...
    if (studentId) {
      ...
      await saveStudentPreferences({
        ...
        preferences: {
          ...existingPreferences,
          creativeActivityPreference: option,
        },
        prompts: {
          ...existingPrompts,
          creativeActivityPreference: textToSpeak,
        },
        buddy: buddy || null,
  ...
  return (
    <>
      <Helmet>
        <title>Creative Activity Preference | Kibundo Interest Selection</title>
        <meta name="description" content="Choose between painting or building in this fun interactive preference selection for kids." />
      ...
      <div className="relative w-full min-h-screen overflow-hidden flex items-center justify-center">
        ...
        >
          ...
          {/* Speech Bubble - at x:14, y:97 relative to component, 205x86 */}
          <div 
            className="absolute"
            style={{
              left: 'clamp(14px, 1.1vw, 14px)',
        ...
        >
          {/* Button 1 - Painting */}
          <button
            onClick={() => handleOptionSelect('Malen')}
            className={`relative overflow-hidden transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-400 flex-shrink-0 ${
              selectedOption === 'Malen' ? 'ring-2 ring-green-500 scale-105' : ''
            }`}
            style={{
            ...
            aria-label="Select Painting"
          >
            {/* Painting illustration - centered */}
            <div 
              className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2"
              style={{
                width: 'clamp(122px, 19.06vw, 244px)',
                height: 'clamp(79px, 12.34vh, 159px)'
              }}
            >
              <img
                src="/images/img_frame_deep_orange_100_03.svg"
                alt="Painting"
          ...
          {/* Button 2 - Building */}
          <button
            onClick={() => handleOptionSelect('Bauen (Lego)')}
            className={`relative overflow-hidden transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-400 flex-shrink-0 ${
              selectedOption === 'Bauen (Lego)' ? 'ring-2 ring-green-500 scale-105' : ''
            }`}
            style={{
              width: isPortrait ? 'clamp(180px, 42vw, 380px)' : 'clamp(280px, 29.7vw, 380px)',
              maxWidth: isPortrait ? 'calc(50% - 8px)' : 'calc(50% - 8px)',
          ...
          >
            {/* Building illustration - centered */}
            <div 
              className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2"
              style={{
                width: 'clamp(86px, 13.44vw, 172px)',
                height: 'clamp(98px, 15.31vh, 196px)'
              }}
            >
              <img
                src="/images/img_isolationsmodus_amber_700_01.svg"
                alt="Building"
                className="w-full h-full object-contain"
                style={{
                  filter: 'drop-shadow(0px 0px 12px rgba(255, 255, 255, 1))'
...
export default CreativeActivityPreference;
