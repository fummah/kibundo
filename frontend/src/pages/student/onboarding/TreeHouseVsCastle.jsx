// src/pages/student/onboarding/TreeHouseVsCastle.jsx
// Based on fortai_app/src/pages/onboarding/TreeHouseVsCastle.jsx
...
const TreeHouseVsCastle = () => {
  ...
  const textToSpeak = "Wo wärst du lieber? In einem Baumhaus oder in einem Schloss.";
  ...
  const handleChoice = async (choice) => {
    ...
    if (studentId) {
      ...
      await saveStudentPreferences({
        ...
        preferences: {
          ...existingPreferences,
          treehouseVsCastle: choice,
        },
        prompts: {
          ...existingPrompts,
          treehouseVsCastle: textToSpeak,
        },
        buddy: buddy || null,
  ...
  const handleBackClick = () => {
    navigate('/student/onboarding/dinosaurs-vs-unicorns', {
  ...
  return (
    <>
      <Helmet>
        <title>Tree House vs Castle | Kibundo Interest Selection</title>
        <meta name="description" content="Choose between tree house and castle in this fun interactive preference selection for kids." />
        <meta property="og:title" content="Tree House vs Castle | Kibundo Interest Selection" />
        <meta property="og:description" content="Choose between tree house and castle in this fun interactive preference selection for kids." />
      </Helmet>
      <div className="relative w-full min-h-screen overflow-hidden flex items-center justify-center">
        ...
        >
          ...
          {/* Speech Bubble - at x:2, y:97 relative to component, 217x111 */}
          <div 
            className="absolute"
            style={{
              left: '2px',
        ...
        {/* Selection Buttons - centered, responsive, positioned lower to not cover buddy */}
        <div 
        ...
        >
          ...
          <button
            ...
            className={`overflow-hidden transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-400 flex-shrink-0 ${
              selectedChoice === 'Baumhaus' ? 'ring-2 ring-green-500 scale-105' : ''
            }`}
            style={{
              width: isPortrait ? 'clamp(180px, 42vw, 395px)' : 'clamp(280px, 30.86vw, 395px)',
              maxWidth: isPortrait ? 'calc(50% - 8px)' : 'calc(50% - 8px)',
              height: 'auto',
              aspectRatio: '395 / 264',
              borderRadius: 'clamp(12px, 1.25vw, 16px)',
              backgroundColor: '#DBEFCE',
              border: '1px solid #C4D678',
              boxShadow: '2px 2px 5px rgba(0, 0, 0, 0.25)',
              boxSizing: 'border-box'
            }}
            aria-label="Select Tree House"
          >
            <div 
              className="relative w-full h-full flex items-center justify-center"
              style={{
                padding: 'clamp(20px, 2.5vh, 34px)'
              }}
            >
              <img
                src="/images/img_frame_red_300_03.svg"
                alt="Tree House"
          ...
          {/* Castle card */}
          <button
            onClick={() => handleChoice('Schloss')}
            className={`overflow-hidden transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-400 flex-shrink-0 ${
              selectedChoice === 'Schloss' ? 'ring-2 ring-green-500 scale-105' : ''
            }`}
            style={{
              width: isPortrait ? 'clamp(180px, 42vw, 395px)' : 'clamp(280px, 30.86vw, 395px)',
              maxWidth: isPortrait ? 'calc(50% - 8px)' : 'calc(50% - 8px)',
...
export default TreeHouseVsCastle;
