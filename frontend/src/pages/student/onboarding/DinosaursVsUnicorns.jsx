// src/pages/student/onboarding/DinosaursVsUnicorns.jsx
// Based on fortai_app/src/pages/onboarding/DinosaursVsUnicorns.jsx
...
const DinosaursVsUnicorns = () => {
  ...
  const textToSpeak = "Was magst Du lieber: Dinosaurier oder Einhörner?";
  ...
  const handleChoiceSelect = async (choice) => {
    ...
    if (studentId) {
      ...
      await saveStudentPreferences({
        ...
        preferences: {
          ...existingPreferences,
          dinosaursVsUnicorns: choice,
        },
        prompts: {
          ...existingPrompts,
          dinosaursVsUnicorns: textToSpeak,
        },
        buddy: buddy || null,
  ...
  return (
    <>
      <Helmet>
        <title>Dinosaurs vs Unicorns | Kibundo Interest Selection</title>
        <meta name="description" content="Choose between dinosaurs and unicorns in this fun interactive preference selection for kids." />
      ...
      <div className="relative w-full min-h-screen overflow-hidden flex items-center justify-center">
        ...
        >
          ...
          {/* Speech Bubble - at x:4, y:97 relative to component, 215x111 */}
          <div 
            className="absolute"
            style={{
        ...
        >
          {/* Dinosaurs card */}
          <button
            onClick={() => handleChoiceSelect('Dinosaurier')}
            className={`overflow-hidden transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-400 flex-shrink-0 ${
              selectedChoice === 'Dinosaurier' ? 'ring-2 ring-green-500 scale-105' : ''
            }`}
            style={{
              width: isPortrait ? 'clamp(180px, 42vw, 405px)' : 'clamp(280px, 31.64vw, 405px)',
            ...
            aria-label="Select Dinosaurs"
          >
            <img
              src="/images/img_dinosaurs_vs_unicorns_dinosaurs.svg"
              alt="Dinosaurs illustration"
          ...
          {/* Unicorns card */}
          <button
            onClick={() => handleChoiceSelect('Einhörner')}
            className={`overflow-hidden transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-400 flex-shrink-0 ${
              selectedChoice === 'Einhörner' ? 'ring-2 ring-green-500 scale-105' : ''
            }`}
            style={{
              width: isPortrait ? 'clamp(180px, 42vw, 405px)' : 'clamp(280px, 31.64vw, 405px)',
              maxWidth: isPortrait ? 'calc(50% - 8px)' : 'calc(50% - 8px)',
            ...
            aria-label="Select Unicorns"
          >
            <img
              src="/images/img_dinosaurs_vs_unicorns_unicorns.svg"
              alt="Unicorns illustration"
              className="w-full h-full object-contain"
            />
          </button>
        </div>
...
export default DinosaursVsUnicorns;
