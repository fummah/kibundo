...
export default function TreasureMap() {
  ...
  const overallPct = Math.min(100, Math.round((unlockedCount / ALL_AREAS.length) * 100));
    ...
    <div className="px-3 md:px-6 py-4 bg-gradient-to-b from-white to-neutral-50 min-h-[100dvh]">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <Title level={4} className="!mb-0">Treasure Map</Title>
        </div>
        <div className="flex items-center gap-2">
          <Tag color="gold"><Crown className="inline w-4 h-4 mr-1" /> {mockProfile.stars} stars</Tag>
          <Tag color="blue"><Trophy className="inline w-4 h-4 mr-1" /> {mockProfile.xp} XP</Tag>
        </div>
      </div>
      ...
      <Card className="rounded-2xl mb-4">
        <div className="flex items-center gap-3">
          ...
          <Button type="primary" className="rounded-xl" onClick={() => navigate("/student/learning")}>
            Continue Learning
          </Button>
        </div>
      </Card>
        ...
        {areas.map((a) => (
          ...
          >
            <Card
              ...
              className={`rounded-2xl overflow-hidden border-0 shadow-sm ${a.unlocked ? "" : "opacity-60"}`}
              bodyStyle={{ padding: 0 }}
            >
              <div className="relative">
                <img src={a.img} alt={a.name} className="w-full h-40 object-cover" />
                {!a.unlocked && (
              ...
              <div className="px-3 py-3">
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {a.skills.slice(0, 3).map(s => (
                    <Tag key={s} className="rounded-full">{s}</Tag>
                  ))}
                </div>
              </div>
            </Card>
      ...
      >
        {active && (
          <div className="space-y-3">
            <img src={active.img} alt={active.name} className="w-full h-40 object-cover rounded-xl" />
            <div>
              <Text type="secondary">Key skills</Text>
              <div className="mt-1 flex flex-wrap gap-2">
                {active.skills.map(s => <Tag key={s} className="rounded-full">{s}</Tag>)}
              </div>
            </div>
            <div>
              <Text type="secondary">Suggested next steps</Text>
              <ul className="list-disc pl-5 mt-1">
                {active.tasks.map(t => <li key={t} className="text-sm">{t}</li>)}
              </ul>
            </div>
            <div className="flex gap-2 pt-1">
              <Button type="primary" className="rounded-xl" onClick={() => { setActive(null); /* route to reading/learning */ }}>
                Start Activity
              </Button>
              <Button className="rounded-xl" onClick={() => setActive(null)}>Close</Button>
            </div>
          </div>
        )}
      </Modal>
