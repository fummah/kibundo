          ...
          {isAuthenticated && (
            <>
              ...
              <div ref={searchRef} className="relative">
                <button onClick={() => setShowSearch(!showSearch)} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800">
              ...
              <div className="relative notification-menu">
                <Tooltip title="Notifications">
                  <Badge count={notificationCount}>
                    <button onClick={() => setNotificationOpen(!notificationOpen)} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800">
                  ...
                  <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-800 shadow-lg rounded-md z-50 text-sm">
                    ...
                    <ul className="divide-y dark:divide-gray-700 max-h-60 overflow-y-auto">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((n) => (
                          <li key={n.id} className="p-3 hover:bg-gray-50 dark:hover:bg-gray-700">{n.message}</li>
                        ))
                      )}
                    </ul>
                  </div>
              ...
              <div className="relative">
                ...
                {userDropdownOpen && (
                  ...
                  >
                    ...
                    {isAdmin && (
                      <button onClick={() => navigate("/admin/reports")} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700">
                        Admin Reports
                      </button>
                    )}
                    {isTeacher && (
                      <button onClick={() => navigate("/teacher/assignments")} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700">
                        My Assignments
                      </button>
                    )}
                    {isStudent && (
                      <button onClick={() => navigate("/student/courses")} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700">
                        My Courses
                      </button>
                    )}
                    {isParent && (
                      <button onClick={() => navigate("/parent/children")} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700">
                        My Children
                      </button>
                    )}

                    <button onClick={goToSettings} className="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
